...
namespace Kinogoblin.Editor.FavoriteAssets
{
    public static class WindowStyler
    {
        ...
        public static void DrawGlobalHeader(EditorWindow window, Texture icon, Color color, string label, string version = "")
        {
            ...
            if (EditorData.Instance.HeadlineStyle != null)
                GUILayout.Label(label, EditorData.Instance.HeadlineStyle);
        ...
        public static void DrawCenteredMessage(EditorWindow window, Texture icon, float msgWidth, float msgHeight, string messsage)
        {
            ...
            EditorGUI.DrawRect(new Rect(rectStartPos.x, rectStartPos.y, outerBoxSize.x, outerBoxSize.y), WindowStyler.clr_White);
            EditorGUI.DrawRect(new Rect(rectStartPos.x + frameWidth, rectStartPos.y + frameWidth, innerBoxSize.x, innerBoxSize.y), WindowStyler.clr_dBlue);
            ...
            GUI.Label(new Rect(labelStartPos.x, labelStartPos.y, textWidth, textHeight), messsage, EditorData.Instance.HeadlineStyle);
        }
    }
}