...
{
    [CustomEditor(typeof(PackageDataManager))]
    [InitializeOnLoad]
    public class PackageDataManagerEditor : UnityEditor.Editor
    {
        public static readonly string ShowedReadmeProjectStateName = "HeurekaGames.PackageDataManager.ShowedReadme";

        static float kSpace = 16f;

        static PackageDataManagerEditor()
        {
        ...
        public static PackageDataManager SelectReadme()
        {
            var ids = AssetDatabase.FindAssets("t:" + typeof(PackageDataManager).ToString());
            ...
            if (ids.Length == 1)
            {
                ...
                return (PackageDataManager)readmeObject;
            }
            else
            {
        ...
        {
            readmeManager = (PackageDataManager)target;
            //populate sections
            readmeManager.sections = Resources.FindObjectsOfTypeAll<PackageData>();
            ...
            List<PackageData> listUniqueEntries = new List<PackageData>();
            foreach (var item in readmeManager.sections)
            {
                ...
                else
                {
                    ...
                    if (item.GetType() == typeof(PackageDataPromo))
                        continue;
                    else
                    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            foreach (var section in readmeManager.sections)
            {
                ...
                if (section.GetType() == typeof(PackageDataVersioned))
                {
                    PackageDataVersioned versionedSection = (PackageDataVersioned)section;
        ...
        private PackageDataManager readmeManager;
